...
function quick_partition (start, end)
{
    ...
    div_update(divs[start],div_sizes[start],"yellow");//Color update
        ...
        for(var j =start + 1; j <= end ; j++ )
        {
            ...
            if (div_sizes[ j ] < piv)
            {
                ...
                div_update(divs[i],div_sizes[i],"#ff4d4d");//Color update
                div_update(divs[j],div_sizes[j],"#ff4d4d");//Color update
                ...
                div_update(divs[j],div_sizes[j],"#ff4d4d");//Height update
    ...
    div_update(divs[start],div_sizes[start],"#ff4d4d");//Color update
    div_update(divs[i-1],div_sizes[i-1],"#ff4d4d");//Color update
    
    var temp=div_sizes[start];//put the pivot element in its proper place.
    div_sizes[start]=div_sizes[i-1];
    div_sizes[i-1]=temp;

    div_update(divs[start],div_sizes[start],"#ff4d4d");//Height update
    div_update(divs[i-1],div_sizes[i-1],"#ff4d4d");//Height update
